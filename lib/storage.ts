...
// Local MomentumItem that matches the legacy storage shape (camelCase)
interface LegacyMomentumItem {
  name: string;
  current: number;
  previous: number;
  deltaPercent: number;
...
function buildMomentum(items: IntelItem[]): LegacyMomentumItem[] {
  const now = Date.now();
